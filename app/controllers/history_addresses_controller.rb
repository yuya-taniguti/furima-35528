class HistoryAddressesController < ApplicationController
  ...
  def history_address
    params.require(:history_address).permit(:postal_code, :regional_id, :municipality, :address, :phone_number, :building_name).merge(user_id: current_user.id, item_id: params[:item_id], token: params[:token])
  end
